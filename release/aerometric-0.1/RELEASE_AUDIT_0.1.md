...
## Production verification addendum — 2026-09-22

PR #1 merged the CC0 Reference Drone and its Release assets to `main` as `dec23a63`. A production-upload dry run from the original checkout then found 14 untracked local planning documents that Git had ignored but Vercel would upload. PR #2 added that directory to `.vercelignore`; the final dry run included 74 files and excluded those documents, Quadrotor V3 assets, local environment files, release duplicates, and old build output. PR #2 merged as `a8a0fc20c01f7723687737c13d72519f91d1561e`; [main CI](https://github.com/JianghuiHu/aerometric/actions/runs/35698746972) passed.

The Git-connected Vercel project then built Production deployment `dpl_Dmsyd1G8Dr1y76ArY8ZBLBMUnVL1` from `main`, assigned [aerometric.vercel.app](https://aerometric.vercel.app), and reported Ready. The site returns HTTP 200 for `/`, the one-entry Community Library index, Reference Drone GLB, Profile, and preview image; `/models/drone_v3.glb` returns 404. Headless Chrome checks against the canonical URL passed default built-in loading and Community Library loading, Chinese/English toggle, color and visibility, rotor and gimbal, mission LEDs, sky/rain/night, PNG/JPG, GLB export, and exported-GLB re-import. Re-import preserved body color, excluded the hidden camera, and retained body geometry dimensions. The GitHub `v0.1.0` Release remains pending at the time of this audit entry.

## Published Release verification — 2026-09-22

The final Release documentation merged to `main` at `d17ba11a9d559c3d7cb07d32043b7651b4c03522`. [Main CI](https://github.com/JianghuiHu/aerometric/actions/runs/35701262979) passed, as did the manually dispatched [public-deployment-gate and source jobs](https://github.com/JianghuiHu/aerometric/actions/runs/35702064311) on that exact SHA. Vercel Production deployment `dpl_7k6s83f3HJuAcSPu6AAgUvPa1BCi` was Ready and assigned to [aerometric.vercel.app](https://aerometric.vercel.app). Production browser dogfood again passed Community Library loading, appearance, rotor, gimbal, six status lights, sky/rain/night, PNG/JPG, and GLB export/re-import with edited color, hidden camera, and unchanged dimensions.

[GitHub Release v0.1.0](https://github.com/JianghuiHu/aerometric/releases/tag/v0.1.0) is published from tag `v0.1.0` at `d17ba11a9d559c3d7cb07d32043b7651b4c03522`. Its three uploaded assets are the AI Model Kit ZIP (9,590 bytes), Blender Helper ZIP (6,499 bytes), and CC0 Reference Drone 01 ZIP (46,682 bytes). The Release is neither draft nor prerelease. Quadrotor V3 assets remain unpublished; V3-dependent tests require a local private fixture and are outside the reproducible public CI run.

## Post-v0.1.0 official V3 deployment — 2026-09-22

The preceding entries describe the frozen `v0.1.0` tag and are preserved as historical facts. After that release, the Quadrotor V3 rights holder confirmed ownership and authorized CC0-1.0 public distribution of its model, materials, and self-authored textures. [PR #5](https://github.com/JianghuiHu/aerometric/pull/5) merged the original V3 geometry, separately licensed model package, Level 1 sidecar Profile, Level 2 glTF extras, and official Library metadata to `main` at `c4c429fcee52691c4bfa86310a1fd6dddf5406ef`. Reference Drone 01 remains a lightweight official validation model. The published `v0.1.0` GitHub Release assets were not rewritten.

The clean-checkout License Gate passed with two indexed CC0 models; Library and Release validators, 25/25 source tests, and Production Build passed. The local full suite passed 63/63. A follow-up made all 63 tests reproducible from the public packages: tests now use the licensed V3, the original dimensional manifest values, and a pre-light geometry fingerprint without requiring the historical private GLB fixture. CI now runs that full suite and the export/re-import verifier. [Main CI run 35747420825](https://github.com/JianghuiHu/aerometric/actions/runs/35747420825) passed before that CI expansion. Vercel Production deployment 6594340252 succeeded and [aerometric.vercel.app](https://aerometric.vercel.app) now serves V3 by default. The public model index contains two official entries; the root-level unindexed `drone_v3.glb` is absent from the clean build.

Production browser verification passed default V3 loading, both Library models, a user-uploaded GLB, Chinese/English, color and visibility, rotors, gimbal, six mission LEDs, sky/rain/night, PNG/JPG, current GLB export, and GLB re-import. A local 1920 × 1080 PNG/JPG, 3840 × 2160 PNG, and transparent PNG check also passed. V3 re-import retained edited color and excluded the hidden camera. `Body_Main`'s local X bounding-box extent differed by about 0.000152 m after standard glTF export/re-import; no overall scale change or clipping was observed. This small serialization difference remains documented rather than described as bit-for-bit geometry preservation.

The performance comparison is in [PERFORMANCE_V3.md](PERFORMANCE_V3.md). On this host in Headless Chrome at DPR 1.5, V3 with Bloom improved from 12.3 to 56.8 FPS, frame time from 81.2 to 17.6 ms, and draw calls from 891 to 403. Source V3 geometry stayed at 177 meshes and 79,708 triangles. This is a controlled local result, not a guarantee for every browser or device. The 552.10 KB shared Three.js production chunk remains a non-blocking optimization item; Bloom is separately lazy-loaded.

## v0.1.1 repository presentation candidate — 2026-09-23

The Studio package is now `0.1.1`; Model Standard, DroneProfile, Library, Validator, Skill, and Blender Helper contracts intentionally remain on the compatible 0.1 line. Root `README.md` is the English canonical product page, with `README.zh-CN.md` and paired Getting Started, AI Skill, Model Standard Overview, Contributing, and release-note pages under `docs/en` and `docs/zh-CN`. Local relative-link validation passes. The README presents the live Studio first, then AI Skill, Blender Creator Kit, official models, compatibility levels, Community Library, developer setup, documentation, contribution rules, and licensing.

Studio now exposes restrained Docs and GitHub links beside the existing language switch. Docs follows the active locale, and the document language attribute is synchronized on first load and after switching. Both locale catalogs contain the same 138 keys. Browser verification loaded the default V3, showed the two header links, and confirmed English and Simplified Chinese labels plus locale-specific documentation URLs without console warnings or errors.

Local candidate gates currently pass: 63/63 full tests, 25/25 source tests, GLB and image export verification, the two-entry Community Library validator, Release validator, public asset/license gate, and Production Build. The build transforms 79 modules; its main application chunk is 182.68 KB uncompressed / 52.57 KB gzip, Bloom remains a 19.86 KB lazy chunk, and shared Three.js remains 552.10 KB / 140.09 KB. The public indexes still contain Quadrotor V3 (default/full feature, CC0-1.0) and Reference Drone 01 (lightweight/validation, CC0-1.0). A clean PR CI result, merged `main` CI, Vercel Production deployment, and creation of the separate `v0.1.1` GitHub Release remain pending until this candidate is reviewed and merged; the existing `v0.1.0` Release is unchanged.

[PR #7](https://github.com/JianghuiHu/aerometric/pull/7) subsequently passed its source checks and Vercel Preview, then merged to `main` as `c608c7a56868a23bbca3b6c051278bd05e911da1`. [Main CI run 35813874411](https://github.com/JianghuiHu/aerometric/actions/runs/35813874411) passed on that exact merge commit. Vercel reported the corresponding Production deployment complete; the canonical site returned HTTP 200 and served bundle `index-Cwak_TPb.js`, which contains the localized GitHub and documentation routes. The English canonical README, Chinese README, and package version `0.1.1` are present on remote `main`. A separate `v0.1.1` GitHub Release has not been created and remains the only planned publication step; the existing `v0.1.0` Release remains unchanged.
