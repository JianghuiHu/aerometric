...
if(direct){const root=path.resolve(path.dirname(fileURLToPath(import.meta.url)),'..'),models=path.join(root,'models'),index=JSON.parse(fs.readFileSync(path.join(models,'index.json'),'utf8')),errors=validateModelLibrary(models,index);if(!errors.length)for(const entry of index.models){const report=await validateModelPackage(path.join(models,entry.model),entry.profile?path.join(models,entry.profile):undefined);if(!report.pass)errors.push(`${entry.id}: ${report.errors.map(issue=>issue.message).join('; ')}`);else if(Number(report.level)<entry.compatibility)errors.push(`${entry.id}: declared Level ${entry.compatibility} exceeds validated Level ${report.level}`);}if(errors.length){console.error(errors.join('\n'));process.exitCode=1;}else console.log(`AEROMETRIC model library ${index.version} valid: ${index.models.length} entries`);}
