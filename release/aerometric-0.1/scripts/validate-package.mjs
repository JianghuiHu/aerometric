...
required.push('docs/model-mapping.md','docs/model-library.md','models/_template/CONTRIBUTION_CHECKLIST.md','.github/PULL_REQUEST_TEMPLATE/model.md','schema/model-library.schema.json','scripts/validate-model-library.mjs','scripts/build-reference-model.mjs','artifacts/AEROMETRIC_Reference_Drone_01_0.1.zip','RELEASE_AUDIT_0.1.md');
for(const file of required)if(!fs.existsSync(path.join(root,file)))fail(`Missing package file: ${file}`);
if(!fs.readFileSync(path.join(root,'schema/aerometric-profile.schema.json')).equals(fs.readFileSync(path.join(root,'kits/AEROMETRIC_AI_Model_Kit/schema/aerometric-profile.schema.json'))))fail('AI Model Kit Profile Schema is out of sync');
